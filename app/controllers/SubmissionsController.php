...
class SubmissionsController extends ActiveController
{
        ...
        return ArrayHelper::merge(parent::behaviors(), [
            'access' => [
                ...
                'rules' => [
                    ...
                    ['actions' => ['view', 'read'], 'allow' => true, 'roles' => ['viewFormSubmissions'], 'roleParams' => function() {
                        $id = Yii::$app->request->get('id', Yii::$app->request->post('id'));
                        $submission = FormSubmission::findOne(['id' => $id]);
                        return ['model' => $submission->form];
                    }],
                    ['actions' => ['email'], 'allow' => true, 'roles' => ['viewFormSubmissions'], 'roleParams' => function() {
                        $form = Form::findOne(['id' => Yii::$app->request->get('id')]);
                        if (!$form) {
                            $submission = FormSubmission::findOne(['id' => Yii::$app->request->post('sid')]);
    ...
    public function actions()
    {
        ...
        $actions['index']['prepareDataProvider'] = function () {
            ...
            if ($model) {
                // Time Zone
                if (!empty($model->submission_timezone)) {
                    Yii::$app->timeZone = $model->submission_timezone;
                }
                // Date Format
                if (!empty($model->submission_dateformat)) {
                    /** @var \kartik\datecontrol\Module $dateControlModule */
                    $dateControlModule = Yii::$app->getModule('datecontrol');
                    $dateControlModule->displaySettings['datetime'] = $model->submission_dateformat;
                }
            }
            ...
            if (isset($q)) {
                ...
                if (!empty($q)) {
                    ...
                    } elseif (preg_match('/(status|STATUS|submission_status|SUBMISSION_STATUS):(\d+)/', $q, $matches)) { // Filter by Submission Status
                        if (!empty($matches[2])) {
                    ...
                    } elseif (preg_match('/(status|Status|STATUS|submission_status|SUBMISSION_STATUS):(.?)(verified|Verified|VERIFIED|confirmed|Confirmed|CONFIRMED)/', $q, $matches)) { // Filter by Submission Status
                        if (!empty($matches[2])) {
                            $status = 7;
                            $query = FormSubmission::find()
                                ->where('form_id=:form_id', [':form_id' => $id])
                    ...
                    } elseif (preg_match('/(status|Status|STATUS|submission_status|SUBMISSION_STATUS):(.?)(unverified|Unverified|UNVERIFIED|unconfirmed|Unconfirmed|UNCONFIRMED)/', $q, $matches)) { // Filter by Submission Status
                        if (!empty($matches[2])) {
                            $status = 1;
                            $query = FormSubmission::find()
                                ->where('form_id=:form_id', [':form_id' => $id])
            ...
            /**
             * Sort Submissions by Form fields
             */
            if (!in_array($sort, ['created_at', '-created_at'])) {

                /** @var FormSubmission[] $entries */
                $entries = $query->all();
                $allModels = [];
                $attributes = [];
                ...
                foreach ($entries as $entry) {
                    ...
                    if (!empty($entry->data)) {
                        $data = is_string($entry->data) ? Json::decode($entry->data) : $entry->data;
                    ...
                    // Save model attribute names
                    // Execute only the first loop
                    if (empty($attributes)) {
    ...
    public function actionRead()
    {
        ...
        try {
            ...
            $success = FormSubmission::updateAll(['new' => 0], ['id' => $id, 'form_id' => $fid]);

            // Response format
            Yii::$app->response->format = Response::FORMAT_JSON;
            $response = [
                'success' => $success,
                'message' => $success ? Yii::t('app', 'Marked as read.') : Yii::t('app', 'It was already read.'),
            ];
