        ...
        if ($post = $request->post()) {
            ...
            try {
                ...
                if (isset($post['action']) && $post['action'] === 'test-email') {
                    ...
                    } elseif ($mailerTransport === 'sendgrid') {
                        $apiKey = $request->post('sendgrid_key');
                        if (!empty($apiKey)) {
                            $settings->set('sendgrid.key', $apiKey);
                            // Save Mailer Transport
                            $settings->set('app.mailerTransport', $mailerTransport);
                            // Show success alert
                            Yii::$app->getSession()->setFlash(
                            ...
                            $settings->set('sendgrid.key', '');
                            // Show success alert
                            Yii::$app->getSession()->setFlash(
                                'danger',
                                Yii::t('app', 'Your Sendgrid Api Key is empty. Try it again.')
                            );
                        }
