                        ...
                        foreach ($models as $model) {
                            if (class_exists($model)) {
                                ...
                                if ((new $model)->hasAttribute('form_id')) {
                                    $results = $model::findAll(['form_id' => $id]);
                                } elseif (method_exists($model, 'findAllByFormID')) {
                                    $results = $model::findAllByFormID($id);
                                }
                                if (empty($results)) {
                                    ...
                                    foreach ($results as $result) {
                                        $recordName = isset($result->name) ? $result->name : '';
                                            ...
                                            'html' => '
                                                <tr>
                                                    <td>
                                                        <a href="'.Url::to(["/addons/".$addOnModel->id]).'">'. $addOnModel->name .'</a>
                                                        <div class="actions" style="float: right">
                                                            <a href="'.Url::to(["/addons/".$addOnModel->id."/admin/view", "id" => $result->id]).'" class="view" title="'.Yii::t('app', 'View') . ' ' . $recordName .'">
                                                                <span class="glyphicon glyphicon-eye-open"></span></a>
                                                            <a href="'.Url::to(["/addons/".$addOnModel->id."/admin/update", "id" => $result->id]).'" class="edit" title="'.Yii::t('app', 'Update') . ' ' . $recordName .'">
                                                                <span class="glyphicon glyphicon-pencil"></span></a>
                                                            <a data-method="post" data-confirm="'.Yii::t('app', 'Are you sure you want to delete this configuration? All data related to this item will be deleted. This action cannot be undone.').'" href="'.Url::to(["/addons/".$addOnModel->id."/admin/delete", "id" => $result->id]).'" class="remove" title="'.Yii::t('app', 'Delete') . ' ' . $recordName .'">
